...
const YouAvatar = ({ selfUser }: { selfUser: any }) => {
  return (
    <div className="group relative flex flex-col items-center group">
      <div className="absolute top-3 opacity-80 text-white pointer-events-none">
        {selfUser ? getUserInitials(selfUser?.profileData.userName) : '...'}
      </div>
      <div
...
const Avatars = ({ otherUsers }: { otherUsers: any[] }) => {
    ...
    <>
      {otherUsers
        ...
        .map((user, index) => {
          ...
          const { userName, avatarColor } = user.profileData
